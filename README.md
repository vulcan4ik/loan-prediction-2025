...
> **ROC-AUC: 0.92322 | F1-score: 0.944 | Recall: 0.89 | 25 признаков + 5-Fold CV**
...
- [x] SHAP анализ для интерпретации  
---
...
### 1. **EDA**
- Анализ распределений дохода, кредитного рейтинга и процента возвратов по кредиту.  
...
### 4. **SHAP анализ для интерпретации**

📊 Использован **SHAP (SHapley Additive Explanations)** для объяснения вклада признаков в предсказания CatBoost.  

**Ключевые наблюдения:**
- `employment_status` — самый важный фактор. Категории *Employed* и *Self-employed* повышают вероятность выплаты кредита, тогда как *Unemployed* и *Student* снижают.  
- `credit_risk_index` — чем выше индекс, тем выше шанс возврата кредита. Низкие значения — сигнал высокого риска.  
- `debt_to_income_ratio` — чем выше долговая нагрузка относительно дохода, тем хуже прогноз.  

📈 На графике зависимости `grade_subgrade × employment_status` видно:
- Классы A–B имеют положительные SHAP-значения → низкий риск.  
- Классы D–F — отрицательные SHAP-значения → высокий риск дефолта.  
- При одинаковом `grade_subgrade` статус занятости корректирует риск:
  - **Работающие (Employed)** — модель “доверяет”, их SHAP ближе к нулю.  
  - **Безработные и студенты** — усиливают риск даже при хорошем рейтинге.

 **Итог:**  
> Модель не только опирается на кредитный рейтинг, но и корректирует прогноз социальными факторами.  
> Даже при одинаковом рейтинге занятость и уровень дохода заметно меняют вероятность выплаты кредита.  
...
v4: + Порог F1 + proba submit           → ROC-AUC = 0.92322 
```
